...
/*   Updated: 2025/12/19 14:41:11 by mcolin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_player_move(t_data *data, char is_x, char move)
{
	...
	next_pos = data->map[data->player.y + move - (is_x * move)][data->player.x + (is_x * move)];
	if (next_pos != '1')
	{
		if (is_x)
			data->player.x += move;
...
{
	mlx_on_event(data->mlx, data->window, MLX_KEYDOWN, key_hook, data);
	mlx_on_event(data->mlx, data->window, MLX_WINDOW_EVENT, window_hook, data);
}
